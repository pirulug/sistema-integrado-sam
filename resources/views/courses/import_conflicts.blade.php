<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __("Resolución de Conflictos de Cursos") }}
            </h2>
            <form method="POST" action="{{ route('courses.import-conflicts.cancel') }}" onsubmit="return confirm('¿Está seguro de que desea descartar los cursos pendientes?');">
                @csrf
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="flex items-start">
                    ...
                    <div class="ms-3 flex-1">
                        <h3 class="text-sm font-semibold text-amber-900 dark:text-amber-200">
                            Revisión de cursos con códigos duplicados o en conflicto
                        </h3>
                        <p class="mt-1 text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                            Se registraron automáticamente <strong>{{ $savedCount }}</strong> cursos sin conflicto. A continuación se presentan <strong>{{ count($conflicts) }}</strong> curso(s) que presentan colisión de código con asignaturas registradas. Puedes editar los valores en cada fila, decidir si actualizar el curso existente, crearlo como nuevo con el código corregido, u omitirlo de la importación.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('courses.import-conflicts.resolve') }}">
                @csrf
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div class="grid grid-cols-2 gap-1">
                                            <input type="number" name="rows[{{ $tempId }}][credits]" value="{{ old('rows.'.$tempId.'.credits', $data['credits'] ?? 0) }}" min="0" placeholder="Créd" class="w-full text-xs rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm" />
                                            <input type="number" name="rows[{{ $tempId }}][hours]" value="{{ old('rows.'.$tempId.'.hours', $data['hours'] ?? 0) }}" min="0" placeholder="Hrs" class="w-full text-xs rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm" />
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select name="rows[{{ $tempId }}][curriculum_id]" class="w-full text-xs rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm">
                                            ...
                                            @foreach ($curriculums as $c)
                                                <option value="{{ $c->id }}" {{ $curriculumId == $c->id ? 'selected' : '' }}>
                                                    {{ $c->name }} ({{ $c->year }})
                                                </option>
                                            @endforeach
                                        </select>
