        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg border border-gray-100 dark:border-gray-700">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('efsrts.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="module" name="module" type="text" class="mt-1 block w-full" :value="old('module')" required autofocus placeholder="ej. Módulo I" />
                            ...
                            <div>
                                ...
                                <x-text-input id="module_name" name="module_name" type="text" class="mt-1 block w-full" :value="old('module_name')" placeholder="ej. Diseño y elaboración de páginas web" />
                            ...
                            <div>
                                ...
                                <select id="period" name="period" class="mt-1 block w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900 text-gray-700 dark:text-gray-300 focus:ring-indigo-500 focus:border-indigo-500 text-sm shadow-sm">
                                    ...
                                    @foreach(['I', 'II', 'III', 'IV', 'V', 'VI'] as $p)
                                        <option value="{{ $p }}" {{ old('period') == $p ? 'selected' : '' }}>Periodo {{ $p }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="hours" name="hours" type="number" min="0" class="mt-1 block w-full" :value="old('hours', 96)" placeholder="ej. 96" />
                                    <x-input-error class="mt-2" :messages="$errors->get('hours')" />
                                </div>
                                <div>
                                    <x-input-label for="credits" :value="__('Créditos')" />
                                    <x-text-input id="credits" name="credits" type="number" min="0" class="mt-1 block w-full" :value="old('credits', 3)" placeholder="ej. 3" />
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="hidden" name="practice_lines" id="practice_lines_json" value="{{ old('practice_lines', '[]') }}" />
                            ...
                            <div class="md:col-span-2 border-t border-gray-200 dark:border-gray-700 pt-4">
                                ...
                                <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                                    @if($curriculums->isEmpty())
                                        <p class="text-sm text-gray-500 dark:text-gray-400">No hay mallas creadas aún. Por favor crea una primero.</p>
                                    @else
                                        @foreach($curriculums as $curriculum)
                                            <label class="inline-flex items-center">
                                                <input type="checkbox" name="curriculums[]" value="{{ $curriculum->id }}" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800" {{ is_array(old('curriculums')) && in_array($curriculum->id, old('curriculums')) ? 'checked' : '' }}>
