        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span class="h5 mb-0">{{ __("Editar Malla Curricular") }}</span>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <input id="name" type="text" class="form-control @error("name") is-invalid @enderror" 
                                       name="name" value="{{ old("name", $curriculum->name) }}" required autofocus>
                                @error("name")
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <input id="year" type="number" class="form-control @error("year") is-invalid @enderror" 
                                       name="year" value="{{ old("year", $curriculum->year) }}" required min="1900" max="2100">
                                @error("year")
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="col-md-6">
                                <select id="career_id" class="form-select @error("career_id") is-invalid @enderror" name="career_id" required>
                                    @foreach ($careers as $career)
                                        <option value="{{ $career->id }}" {{ old("career_id", $curriculum->career_id) == $career->id ? "selected" : "" }}>
                                            {{ $career->name }}
                                        </option>
                                    @endforeach
                                </select>
