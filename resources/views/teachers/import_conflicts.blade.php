<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __("Resolución de Conflictos de Importación de Profesores") }}
            </h2>
            <form method="POST" action="{{ route('teachers.import-conflicts.cancel') }}" onsubmit="return confirm('¿Está seguro de que desea descartar todos los registros pendientes?');">
                @csrf
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="flex items-start">
                    ...
                    <div class="ms-3 flex-1">
                        <h3 class="text-sm font-semibold text-amber-900 dark:text-amber-200">
                            Revisión de profesores duplicados o similares
                        </h3>
                        <p class="mt-1 text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                            Se registraron automáticamente <strong>{{ $savedCount }}</strong> profesores sin conflicto. A continuación se presentan <strong>{{ count($conflicts) }}</strong> registro(s) que presentan duplicidad de DNI, código o correo institucional. Puedes editar los valores en cada fila, decidir si actualizar el registro existente, crearlo como nuevo con los datos editados, u omitirlo de la importación.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('teachers.import-conflicts.resolve') }}">
                @csrf
                ...
                <div class="space-y-6">
                    @foreach ($conflicts as $conflict)
                        @php
                            ...
                            $existingId = $conflict["existing_teacher_id"];
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden" x-data="{ action: '{{ $defaultAction }}' }">
                            ...
                            <div class="p-5" x-show="action !== 'ignore'">
                                <input type="hidden" name="rows[{{ $tempId }}][existing_teacher_id]" value="{{ $existingId }}" />
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">
                                            Código Profesor <span class="text-red-500">*</span>
                                        </label>
                                        <input type="text" name="rows[{{ $tempId }}][teacher_code]" value="{{ old('rows.'.$tempId.'.teacher_code', $data['teacher_code'] ?? '') }}" required class="w-full text-xs rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm" />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="rows[{{ $tempId }}][phone]" value="{{ old('rows.'.$tempId.'.phone', $data['phone'] ?? '') }}" class="w-full text-xs rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm" />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="rows[{{ $tempId }}][mobile]" value="{{ old('rows.'.$tempId.'.mobile', $data['mobile'] ?? '') }}" class="w-full text-xs rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm" />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="date" name="rows[{{ $tempId }}][hire_date]" value="{{ old('rows.'.$tempId.'.hire_date', $data['hire_date'] ?? date('Y-m-d')) }}" required class="w-full text-xs rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm" />
                                    </div>
                                </div>
                            </div>
