<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg border border-gray-200 dark:border-gray-700/80">
                <div class="p-6 text-gray-900 dark:text-gray-100" x-data="studentEditForm({
                    document_type: '{{ old('document_type', $student->document_type ?? 'DNI') }}',
                    dni: '{{ old('dni', $student->dni) }}',
                    student_code: '{{ old('student_code', $student->student_code) }}',
                    first_name: '{{ old('first_name', $student->first_name) }}',
                    paternal_last_name: '{{ old('paternal_last_name', $student->paternal_last_name) }}',
                    maternal_last_name: '{{ old('maternal_last_name', $student->maternal_last_name) }}',
                    institutional_email: '{{ old('institutional_email', $student->institutional_email) }}',
                    personal_email: '{{ old('personal_email', $student->personal_email) }}',
                    mobile: '{{ old('mobile', $student->mobile) }}',
                    phone: '{{ old('phone', $student->phone) }}',
                    admission_date: '{{ old('admission_date', $student->admission_date ? \Carbon\Carbon::parse($student->admission_date)->format('Y-m-d') : '') }}',
                    graduation_date: '{{ old('graduation_date', $student->graduation_date ? \Carbon\Carbon::parse($student->graduation_date)->format('Y-m-d') : '') }}',
                    existingPhotoUrl: '{{ $student->photo_url }}',
                    hasPhotoPath: {{ $student->photo_path ? 'true' : 'false' }}
                })">
                    ...
                    <form method="POST" action="{{ route('students.update', $student) }}" enctype="multipart/form-data" class="space-y-6" @submit="handleSubmit($event)">
                        @csrf
                        @method('PUT')

                        <!-- Photo Upload / Edit Section with JS validation -->
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input id="study_program" name="study_program" type="text" class="mt-1 block w-full rounded border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-700/60 text-gray-700 dark:text-gray-300 cursor-not-allowed text-sm shadow-sm font-semibold" value="{{ $student->study_program }}" readonly required />
    ...
    <script>
        function studentEditForm(initialData) {
            return {
                form: {
